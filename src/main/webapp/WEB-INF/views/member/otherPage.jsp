        ...
        <section class="b_inner">
            ...
            <div class="hori_cont">
                <div class="profile_wrap">
                    <div class="profile_img">
                        <img src="resources/profileImg/${userMypage.userProfileName }" alt="착한호랑이">
                    </div>
                </div>
                ...
                <div class="detail">
                    <div class="top">
                        ...
                        <c:choose>
                        <c:when test="${checkFol == 'OK'}">
                        <a href="#" class="followingController" onclick="unfollow()" style="color: red; width: 150px">팔로우취소</a>           
                        </c:when>
                        <c:otherwise>
                        <a href="#" class="followingController" onclick="follow()" style="color: blue; width: 150px">팔로우</a>       
                      </c:otherwise>
                        </c:choose>
                        <a href="directmsg" style="width: 150px" >메세지 보내기</a>
                    </div>
            ...
            <c:forEach var="plist" items="${postList}">
                ...
                <c:set var="thumbNailType" value="${fn:length(thumbNail)-1}"/>
                    ...
                    <a href="boardView?postNum=${plist.postNum }">
                    <video controls autoplay loop muted style="width:300px; height:300px;">                    
	...
	function follow(){
		...
		$.ajax({
			...
			success:function(result){
				console.log(result);
				if(result == "OK"){
					$(".followingController").removeAttr('onclick');
					$(".followingController").text("팔로우취소");
					$(".followingController").attr('onclick', "unfollow()");
 					$(".followingController").css('color','red');
					alert("팔로우 하셨습니다.");
					location.reload();
				}
			},
	...
	function unfollow(){
		...
		$.ajax({
			...
			success:function(result){
				...
				if(result == "OK"){
					$(".followingController").removeAttr('onclick');
					$(".followingController").text("팔로우");
					$(".followingController").attr('onclick', "follow()");
 					$(".followingController").css('color','blue');
					alert("팔로우를 취소 하셨습니다.");
					location.reload();
				}
			},
