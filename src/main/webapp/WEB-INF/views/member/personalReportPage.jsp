...
<section id="container">
    ...
    <div id="main_container">
        ...
        <section class="b_inner">
                ...
                <table>
                	...
                	<c:forEach var="pList" items="${postList}">
                		<tr>
                			...
                			<th>
                			<button class="waring_btn" style="cursor:pointer;" onclick="deletePostModal('${pList.postNum }')">삭제</button>
                			</th>
                		</tr>
                	</c:forEach>
                	</tbody>
            ...
            <button class="waring_btn" onclick="reportList()">모두보기</button>
            </div>
            	...
            	<c:choose>
							...
							<c:when test="${pageDTO.page > 1 }">
								<li class="paginate_button page-item previous">
									<a href="personalReport?page=${pageDTO.page-1}&&nickName=${memberDTO.nickName}&&postWriter=${postWriter}" class="page-link">이전</a>
								</li>
							</c:when>
						</c:choose>
							...
							<c:choose>
								...
								<c:otherwise>
									<li class="paginate_button page-item">
										<a href="personalReport?page=${pageNum}&&nickName=${memberDTO.nickName}&&postWriter=${postWriter}" class="page-link">${pageNum}</a>
									</li>
								</c:otherwise>
							</c:choose>
						</c:forEach>	
						...
						<c:choose>
							<c:when test="${pageDTO.page < pageDTO.maxPage }">
								<li class="paginate_button page-item next">
									<a href="personalReport?page=${pageDTO.page + 1}&&nickName=${memberDTO.nickName}&&postWriter=${postWriter}" class="page-link">다음</a>
								</li>
							</c:when>
							<c:when test="${pageDTO.page >= pageDTO.maxPage }">
...
<div class="modal fade" id="deletePostModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button class="waring_btn" type="button" onclick="deletePost()">삭제하기</button>
				<button class="waring_btn" type="button" onclick="cancelDelete()">취소</button>
			</div>
		</div>
	</div>
</div>
...
function showCC22(commentNum){
	...
	function reportList(){
		location.href = "goAdminPage?nickName=${memberDTO.nickName}";
	}
